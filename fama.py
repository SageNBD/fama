...
for asset in assets:
    # Sentiment
    df_pred_sent[asset] = df[asset].filter(['Pred Score', 'Returns']) 
    df_pred_sent[asset].plot(x="Pred Score", y="Returns", style='o', title=asset)  
    df_corr_sent[asset] = df_pred_sent[asset].corr()
    # Trends
    df_pred_trend[asset] = df[asset].filter(['Returns', asset]) 
